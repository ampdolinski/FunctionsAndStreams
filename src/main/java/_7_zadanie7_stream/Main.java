...
//        to samo, tylko synktatycznie (referencyjnie, funkcyjnie, typowo - komparator z javy)
    ...
    public static List<Post> metoda4_dwaOstatniePosty() {
        return POSTY.stream().skip(POSTY.size()-2).collect(Collectors.toList());
    }

    public static List<Post> metoda5_postyMniejNiz5000Znakow() {
    ...
    public static boolean metoda8_WeryfikacjaPostyWiecejNiz5000Znakow() {
        return POSTY.stream().anyMatch(post -> post.getLiczbaZnakow() > 5000);
    }

    public static boolean metoda9_WeryfikacjaZadenZPostowNieWiecejNiz5Tagow() {
...
public static Optional<Post> metoda11_JakikolwiekPostZElemelek() {
...
//jako lambda->     return POSTY.stream().map(post -> post.getTemat()).collect(Collectors.toList());
...
public static List<Komentarz> metoda14_WszystkieKomentarzeZWszystkichPostow() {
...
public static List<String> metoda15_WszystkieUnikalneTagiMalymiLiterami() {
        ...
        Stream<String> tagiZWszystkichPostow = POSTY.stream().flatMap(post -> {
            String[] tagi = post.getTagi().toLowerCase().split(",");
            return Arrays.stream(tagi);
        })
                .distinct();
        return tagiZWszystkichPostow.collect(Collectors.toList());
}


    public static void main(String[] args) {

        metoda15_WszystkieUnikalneTagiMalymiLiterami().forEach(System.out::println);

        //        metoda1_wypiszTytuly();
//        metoda2_sortuj();
//        metoda3_dwaPierwszePosty();
//        metoda4_dwaOstatniePosty().forEach(post -> System.out.println(post.getTemat()));
//        metoda5_postyMniejNiz5000Znakow().forEach(post -> System.out.println(post));
//        System.out.println(metoda8_postyWiecejNiz5000Znakow());
//        System.out.println(metoda9_postyNieWiecejNiz5Tagow());
...
//        System.out.println(metoda14_WszystkieKomentarzeZWszystkichPostow());
    }
